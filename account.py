...
class CurrentAccount(Account):
    def __init__(self, _account_number, _account_holder_id, initial_balance=0.0, overdraft_limit=0.0):
        super().__init__(_account_number, _account_holder_id, initial_balance)
    ...
    def withdraw(self, amount):
        if amount > 0 and (self.initial_balance - amount) >= -self.overdraft_limit:
            self.initial_balance -= amount
            return True
        return False
